...
class AefiEdit : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        enableEdgeToEdge()
        setContentView(R.layout.activity_aefi_edit)
        // Initialize RetrofitCallsFhir
        retrofitCallsFhir = RetrofitCallsFhir()

        // Load the questionnaire JSON
        questionnaireJsonString = getStringFromAssets("AEFI.json")
        ...
        supportFragmentManager.setFragmentResultListener(
            ...
            Log.d("aefiEdit", "Submit request received")
    ...
    private suspend fun fetchAndPopulateQuestionnaireResponse(responseId: String) {
        ...
        retrofitCallsFhir.fetchQuestionnaireResponse(responseId, object : Callback<ResponseBody> {
                    ...
                    if (questionnaireResponseString != null) {
                        CoroutineScope(Dispatchers.Main).launch {
                            try {
                                ...
                                Log.e("aefiEdit", "Error populating questionnaire", e)
                                Toast.makeText(this@AefiEdit, "Error populating questionnaire", Toast.LENGTH_SHORT).show()
                            }
                        }
                    } else {
                        CoroutineScope(Dispatchers.Main).launch {
                            Toast.makeText(this@AefiEdit, "Failed to retrieve the response data.", Toast.LENGTH_SHORT).show()
                        }
                    }
                } else {
                    CoroutineScope(Dispatchers.Main).launch {
                        Toast.makeText(this@AefiEdit, "Failed to fetch the questionnaire response: ${response.message()}", Toast.LENGTH_SHORT).show()
                        Log.e("aefiEdit", "Failed to fetch response. Response code: ${response.code()}")
                    }
                }
            }
            ...
            override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                CoroutineScope(Dispatchers.Main).launch {
                    Toast.makeText(this@AefiEdit, "Error occurred while fetching: ${t.message}", Toast.LENGTH_SHORT).show()
    ...
    private fun populateQuestionnaireFragment(questionnaireResponse: QuestionnaireResponse) {
        try {
            ...
            Log.d("aefiEdit", "Questionnaire response populated successfully.")
        } catch (e: Exception) {
            Log.e("aefiEdit", "Error initializing the questionnaire fragment or ViewModel", e)
        ...
        if (updatedQuestionnaireResponse != null) {
            ...
            retrofitCallsFhir.updateQuestionnaireResponse(responseId, updatedResponseString, object :
                ...
                override fun onResponse(call: Call<ResponseBody>, response: Response<ResponseBody>) {
                    if (response.isSuccessful) {
                        CoroutineScope(Dispatchers.Main).launch {
                            Toast.makeText(this@AefiEdit, "Data updated successfully.", Toast.LENGTH_SHORT).show()
                            // End the activity after successful submission
                            finish()
                        }
                    } else {
                        CoroutineScope(Dispatchers.Main).launch {
                            Toast.makeText(this@AefiEdit, "Failed to update data: ${response.message()}", Toast.LENGTH_SHORT).show()
                        }
                    }
                }
                ...
                override fun onFailure(call: Call<ResponseBody>, t: Throwable) {
                    CoroutineScope(Dispatchers.Main).launch {
                        Toast.makeText(this@AefiEdit, "Error occurred while updating: ${t.message}", Toast.LENGTH_SHORT).show()
                    }
                }
            })
        } else {
